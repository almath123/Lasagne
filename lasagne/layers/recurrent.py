...
class GRULayerAttn(MergeLayer):
    ...
    operations in a single dot product.
    """
    def __init__(self, incoming, num_units, encoder_hout,
                 resetgate=Gate(W_cell=None),
                 updategate=Gate(W_cell=None),
                 hidden_update=Gate(W_cell=None,
                                    nonlinearity=nonlinearities.tanh),
        ...
        # will just provide the layer input as incomings, unless a mask input
        # or initial hidden state was provided.
        incomings = [incoming, encoder_hout]
        self.mask_incoming_index = -1
        self.hid_init_incoming_index = -1
        if mask_input is not None:
            incomings.append(mask_input)
        ...
        # Initialize parent layer
        super(GRULayerAttn, self).__init__(incomings, **kwargs)
    ...
    def get_output_for(self, inputs, **kwargs):
        ...
        def step(input_n, hid_previous, *args):
            ...
            input_n = input_n + context

            #attn_p = theano.printing.Print("attn", ("shape",))(attn)
            #attn_p = theano.printing.Print("attn")(attn)
            #input_n += attn_p.sum() * 0.000000000000001
        ...
        def step_masked(input_n, mask_n, hid_previous, *args):
            [hid, attn] = step(input_n, hid_previous, *args)

            # Skip over any input with mask 0 by copying the previous
            # hidden state; proceed normally for any input with mask 1.
            hid = T.switch(mask_n, hid, hid_previous)

            # mask out the attention
            attn = attn * mask_n
...
class GRULayerESM(MergeLayer):
    ...
    operations in a single dot product.
    """
    def __init__(self, incoming, num_units, vocab_size,
                 resetgate=Gate(W_cell=None),
                 updategate=Gate(W_cell=None),
                 hidden_update=Gate(W_cell=None,
                                    nonlinearity=nonlinearities.tanh),
        ...
        # Initialize parent layer
        super(GRULayerESM, self).__init__(incomings, **kwargs)
    ...
    def get_output_for(self, inputs, **kwargs):
        ...
        # Create single recurrent computation step function
        # input__n is the n'th vector of the input
        def step(input_n, hid_previous, sm_out, *args):
            # Compute W_{hr} h_{t - 1}, W_{hu} h_{t - 1}, and W_{hc} h_{t - 1}
            hid_input = T.dot(hid_previous, W_hid_stacked)
...
class GRULayerESMGum(MergeLayer):
    r"""
    lasagne.layers.recurrent.GRULayer(incoming, num_units,
    resetgate=lasagne.layers.Gate(W_cell=None),
    updategate=lasagne.layers.Gate(W_cell=None),
    ...
    def __init__(self, incoming, num_units, vocab_size,
        ...
        # Initialize parent layer
        super(GRULayerESMGum, self).__init__(incomings, **kwargs)
...
class GRULayerESMGumAttn(MergeLayer):
    r"""
    lasagne.layers.recurrent.GRULayer(incoming, num_units,
    resetgate=lasagne.layers.Gate(W_cell=None),
    updategate=lasagne.layers.Gate(W_cell=None),
    ...
    def __init__(self, incoming, num_units, vocab_size, encoder_hout, enc_in_mask,
        ...
        # Initialize parent layer
        super(GRULayerESMGumAttn, self).__init__(incomings, **kwargs)
        ...
        self.Wemb = self.add_param(Wemb, (vocab_size, num_units), name="Wemb")
        self.Wsm = self.add_param(Wsm, (num_units*2, vocab_size), name="Wsm")
        self.bsm = self.add_param(bsm, (vocab_size,), name="bsm", regularizable=False)
        self.word_input_zero = self.add_param(word_input_zero, (1, vocab_size,), 
                name="word_input_zero", regularizable=False, trainable=self.teacher_force)
    ...
    def get_output_for(self, inputs, **kwargs):
        ...
        def step(input_n, gb, hid_previous, sm_out, *args):
            ...
            hid_cont = T.concatenate([hid, context], axis=1)

            num_leading_axes = 1
            #hid_sm = hid.flatten(num_leading_axes + 1)
            sm_out = T.dot(hid_cont, Wsm) + bsm
            return hid, sm_out
